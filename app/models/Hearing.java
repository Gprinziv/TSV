...
public class Hearing {
   ...
   public static Hearing getOne(Date date, String bid, Integer cid) {
      ...
      if ((ret = (Hearing) Cache.get(CACHE_PREFIX + cid.toString() +
            bid + date.toString())) != null) {
         return ret;
      }
      ...
      try {
         ...
         if (res.next()) {
            ret = new Hearing(res.getDate("date"), res.getString("bid"),
                  res.getInt("cid"));
            Cache.set(CACHE_PREFIX + cid.toString() + bid + date.toString(),
                  ret);
         }
   ...
   public List<Utterance> getTranscript() {
      ...
      try {
         ...
         while (res.next()) {
            ...
            ret.add(new Utterance(res.getString("html"), res.getInt("time"), speaker));
         }
